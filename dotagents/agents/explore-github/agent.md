...
1. Identify the repository (owner/name) from the caller's prompt or the tools'
   results. Query the relevant state with the read tools: commits via
...
- Run a write tool (create_branch, create_or_update_file, push_files,
  create_pull_request, merge_pull_request, issue_write, add_issue_comment,
  actions_run_trigger) or take corrective action. None of them are in this
  agent's allowlist, even though the github server registers them.
