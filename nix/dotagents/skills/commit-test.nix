...
let
  ...
  # in the whole-tree package at $out/agents/<name>.md — all built from
...
in
{
  ...
  options.dotagents.subagents = {
    commit = lib.mkOption {
      type = lib.types.path;
      description = "opencode subagent definition for commit (built from ../dotagents/agents/commit.md).";
    };
    test = lib.mkOption {
      type = lib.types.path;
      description = "opencode subagent definition for test (built from ../dotagents/agents/test.md).";
    };
  };
  ...
  config = {
    ...
    dotagents.subagents.commit = lib.mkDefault "${local.whole-tree}/agents/commit.md";
    dotagents.subagents.test = lib.mkDefault "${local.whole-tree}/agents/test.md";
  };
}
